...
exports.resolveLocationFromUrl=fn(async(url)=>{
  const GK='AIzaSyAgorCtU_fo1a9tXkuiQgTwOZ0iQ7wJzEE';
  ...
  let state=null,address='',category='',lat=coords?coords.lat:null,lng=coords?coords.lng:null;
  ...
  if(name){
    try{const su='https://maps.googleapis.com/maps/api/place/textsearch/json?query='+encodeURIComponent(name)+(coords?('&location='+coords.lat+','+coords.lng+'&radius=20000'):'')+'&key='+GK;
      const sj=await (await fetch(su)).json();const p=(sj&&sj.results&&sj.results[0])||null;
  ...
  if(lat!=null&&lng!=null){
    try{const gu='https://maps.googleapis.com/maps/api/geocode/json?latlng='+lat+','+lng+'&key='+GK;
      ...
      if(r0){if(!address)address=r0.formatted_address||'';if(!state)state=stFromComps(r0.address_components);}
    }catch(e){}
  }
  if(!state)state=stFromAddr(address);
  if(!name&&address)name=address.split(',')[0];
  if(lat==null||lng==null)return{success:false,message:'Could not extract coordinates. Use the full URL from the browser address bar.'};
  return{success:true,lat:lat,lng:lng,state:state,stateRaw:state,address:address,suggestedName:name,category:category,mapsUrl:'https://www.google.com/maps?q='+lat+','+lng};});
